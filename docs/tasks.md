...
| T-010 | ビジュアルサイトビルダー P0（MVP）: スキーマ・core/render・基本セクション種別・基本編集・自動保存・プレビュー・ZIPエクスポート・Node CLI | 高 | 実装中 | developer | 1回目修正でCSSインジェクション対策は解消。href/URLのXSS対策が制御文字（タブ/改行/CR）によるフィルタバイパスで未解消のため再修正中（2回目） |
