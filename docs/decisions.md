...
## D-008: homepage-editリポジトリを「ビジュアルサイトビルダー」の個別アプリ実装用リポジトリとする

- 日付: 2026-08-04
- 状態: 採用

### 背景
- 本リポジトリはD-002が想定していた運用（テンプレートをコピーして新規プロジェクトの雛形とする）に従い、project001/teate1122テンプレートから新規作成された。
- Issueにより、teate1122（キャンドルブランド個人ホームページ）とは独立したプロジェクトとして、スマートフォンだけでサイトを編集・公開できるビジュアルサイトビルダー（ノーコードWebサイトビルダー）を設計・実装する依頼が来た。

### 決定
- 本リポジトリ（homepage-edit）の役割を、「ビジュアルサイトビルダー」の個別アプリ実装用リポジトリと定義する。CLAUDE.md冒頭の「Project Role」（汎用テンプレート説明）はテンプレート由来のまま残すが、実質的にはD-002が想定する「テンプレートからコピーされた個別アプリ用リポジトリ」として運用する。
- 開発ルール・フロー（Manager-Planner-Developer-Reviewer、tasks/progress/decisions運用、Ponytail原則、Model Routing）はそのまま維持し、アプリの実装はsrc/等の新規ディレクトリに追加する。
- teate1122リポジトリとは別リポジトリとして独立させる（Issue要件通り）。将来的にteate1122以外の複数サイトを管理する機能を見据えた設計とする。

### 理由
- User（Issue発行者）からの明示的な指示であり、Managerが独断で仕様を変更しない原則（CLAUDE.md「Project Role」）に基づき、Issueの指示を優先する。
- D-002の運用モデル（テンプレート→コピー→個別アプリ実装）そのものであり、teate1122のD-008のような既存方針との衝突はない（本リポジトリは元々この用途のために作られた）。

### 影響
- 今後、本リポジトリにビジュアルサイトビルダーの実装コードが追加される。
- 本リポジトリを他プロジェクトの雛形として再利用することは想定しない（既に個別アプリ用リポジトリとして運用されるため）。

---

## D-009: ビジュアルサイトビルダーのアーキテクチャ・データ構造・段階的実装スコープを承認する

- 日付: 2026-08-04
- 状態: 採用

### 背景
- Plannerが、システム全体アーキテクチャ・データ構造・ディレクトリ設計・UI/UX設計・ワイヤーフレーム・技術選定・段階的実装スコープ（P0〜P3）を提案した。
- User承認にあたり4つの論点（GitHub公開機能の扱い、teate1122との関係、ピンチ操作の実装方針、全体アーキテクチャ）を確認した。

### 決定
- **アーキテクチャ**: エディタ（ブラウザ内静的PWA）とNode CLIが同一の`core/render`（依存ゼロESモジュール）を共有する「同型レンダラー」方式を採用。サイトデータはJSON（`site.json` + `pages/*.json`）を単一のソース・オブ・トゥルースとし、HTMLは常に生成物とする（HTML直接編集は行わない）。
- **技術選定**: フレームワーク非依存を徹底。WYSIWYG編集・Undo/Redo・状態管理・IndexedDB・並び替え等は全て軽量な自前実装とし、ProseMirror等の重量ライブラリは採用しない。
- **ディレクトリ構成**: `editor/`（静的PWA）、`core/`（ブラウザ/Node共有）、`core/sections/`（セクションごとにdefine.js/render.js/style.cssの3点セットで自己完結、拡張はディレクトリ追加のみ）、`sites/<siteId>/`、`build/`、`templates/`。
- **ピンチ操作**: Issue原文の「ピンチで自由に画像サイズ変更」から、Planner提案の「焦点(focal)＋ズーム(zoom)調整」方式に変更する。理由: 自由リサイズはレスポンシブレイアウトを構造的に壊すため、セクションベース設計（制約）と矛盾する。アスペクト比を保ったまま画像の見せ方だけを調整する方式にすることで、指の操作感を維持しつつ表示崩れを防ぐ。
- **GitHub公開機能**: P0（MVP）ではZIPエクスポート＋手動アップロードに留め、GitHub API直接公開（Personal Access Tokenのブラウザ保存を伴う）はP2で実装する。理由: PATのブラウザ保存はセキュリティ上の論点があり、エディタ本体・データ構造・レンダラーというコア体験を先に固めてから、認証・公開UXの設計に十分な時間をかけるため。
- **teate1122との関係**: 当初Planner提案はP3（最終フェーズ）でのインポート検討だったが、User要望により前倒しする。P0のセクション種別（hero/text/image+text/contact-social）に加えて、teate1122の実際のページ構成（活動セクション＝キャンドル制作/イベント出店/ワークショップ、SNS中段配置等）に対応できるセクション種別（activity-cards、event-list等）をP0〜P1の早い段階で設計・実装し、teate1122のコンテンツ（`/home/user/teate1122/src/content/`, `src/pages/`）をJSONへ変換するインポートスクリプトをP1中に用意することを目指す（具体的な優先順位はDeveloperの実装時に再調整可能とする）。
- **段階的実装スコープ**: P0（MVP: スキーマv1・`core/render`・基本セクション種別・テキスト/色/画像編集・自動保存・プレビュー・ZIPエクスポート・Node build CLI・テンプレ1種）→ P1（並び替え・Undo/Redo・テーマプリセット・PWAオフライン・WebP最適化、teate1122インポート）→ P2（GitHub API公開・複数ページ・ギャラリー・アニメーション・ダークモード）→ P3（複数サイト管理UI・テンプレート7種・SEO詳細）の順で承認。まずP0から着手する。

### 理由
- 詳細な比較検討・代替案はPlannerの提案（本セッションの報告）を参照。Managerは、User承認済みの4論点を踏まえて上記の通り確定した。
...
## D-011: トップページへのコンテンツ集約（1ページサイト化）とnav href対応

- 日付: 2026-08-07
- 状態: 採用

### 背景
- Userから、現状の複数ページ構成（home/about/activities/contact/privacy）ではトップページの情報量が薄く、訪問者に事業内容が伝わりにくいという指摘があった。プロフィール・活動内容・お問い合わせを全てトップページのセクションとして統合し、ハンバーガーメニュー（グローバルナビ）は各セクションへのアンカー遷移（`/#profile`等）に変更することとした。

### 決定
- **nav.itemsの拡張方式**: `site.json`の`nav.items`を、既存の`pageId`方式に加えて`href`方式を許容する最小限の汎用拡張とした（`anyOf`で`pageId`または`href`のいずれか必須）。`href`が指定された場合は`resolveUrl(item.href)`をそのまま使い、`#`始まりのアンカーには`aria-current="page"`を付与しない。
- **T-013（複数ページ管理UI）には踏み込まない**: teate1122のデータは今回`href`方式のみで用が足りるため、Planner・複数ページ切替導線等の汎用UIはYAGNIとして見送り、schemaレベルの最小拡張に留めた。
- **セクションへのアンカーID付与**: `core/render/render-page.js`の`<section>`タグに`id="${section.id}"`を追加し、アンカーリンクの遷移先として機能させる。既存UI（構成シート等）が参照する`data-section-id`属性はそのまま残す。
- **home.jsonの再編**: 旧`about.json`/`activities.json`/`contact.json`の内容をhome.jsonの12セクションへ統合し、旧3ページのJSONファイルは削除した。`pages`配列からも除去（`["home", "privacy"]`）。
- **PWAプリキャッシュリストの追随修正**: ページファイル削除に伴い`editor/sw.js`の`APP_SHELL`（プリキャッシュ対象）に残っていた削除済みファイル参照を除去し、`CACHE_VERSION`をv1→v2に更新した（`cache.addAll`は一部リソースの取得失敗で全体が失敗するため、放置すると既存のオフライン機能が丸ごと壊れる）。

### 理由
- teate1122の実データはページごとの動的な追加・削除を必要としない固定的な1ページ構成であり、`pageId`解決ロジックを汎用的に拡張する（例: 複数ページ管理UI一式）よりも、`href`直接指定を許容する最小拡張の方が実装・レビューコストに見合う（YAGNI）。
- セクションIDをHTMLの`id`属性としてそのまま公開することで、ブラウザ標準のアンカーリンク（`<a href="#id">`）だけでスムーズスクロール遷移が実現でき、追加のJSは不要。

### 影響
- teate1122は実質的に1ページサイト（トップページ＋プライバシーポリシー）になる。将来的に複数ページ構成へ戻す・T-013で複数ページ管理UIを実装する場合は、`nav.items`の`pageId`方式が引き続き利用可能なため後方互換は保たれる。
- `tools/import-teate1122.js`は5ページ構成を前提にしたままであり、再実行するとT-020での統合内容が失われる。同スクリプトの改修は今回のスコープ外とし、冒頭コメントで注意喚起するに留めた。
